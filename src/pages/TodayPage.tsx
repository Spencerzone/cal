...
export default function TodayPage() {
  ...
  // Per-slot manual timing overrides for the currently displayed weekday
  const manualSlotTimings = useMemo(() => {
    const weekday = format(dateLocal, "EEE");
    return (rollingSettings?.slotTimings ?? {})[weekday] ?? {};
  }, [dateLocal, rollingSettings]);
  ...
  const currentNext = useMemo(() => {
    const realEvents = cells
      ...
      .map((x) => {
        ...
        if (cell.kind === "template") {
          ...
          // cell.e may be null when only manualSlotTimings has timing for this slot
          const timing = (cell.e as CycleTemplateEvent | undefined) ??
            (x.slotId ? manualSlotTimings[x.slotId] : undefined);
    ...
    return { current, next };
  }, [cells, now, dateLocal, subjectById, templateBySlot, manualSlotTimings]);
        ...
        >
          <tbody>
            {cells.map(({ block, slotId, cell }) => {
              ...
              const manualSlotTiming = slotId ? manualSlotTimings[slotId] : undefined;
              const timeText =
                cell.kind === "template"
