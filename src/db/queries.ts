...
export function todayRangeUtc(localDate: Date = new Date()): {
  startUtc: number;
  endUtc: number;
} {
  ...
  return {
    startUtc: fromZonedTime(s, TZ).getTime(),
    endUtc: fromZonedTime(e, TZ).getTime(),
  };
}

export function weekRangeUtc(localDate: Date = new Date()): {
  startUtc: number;
  endUtc: number;
} {
  const zoned = toZonedTime(localDate, TZ);
...
export async function getLessonsForSubject(
  ...
  const code = (
    typeof yearOrCode === "string" ? yearOrCode : (maybeCode ?? "")
  ).toUpperCase();
